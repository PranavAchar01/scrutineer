...
// A different set of shots every run, in a different order, held for different lengths.
// Cycling one fixed sequence from the same opening shot made ten different circuits look
// like ten takes of the same footage. Seeded on the run so a seek reproduces the cut.
const SHOTS = ['CHASE', 'TV', 'HELI', 'ONBOARD', 'STUDIO'];
function cameraPlan(run) {
  const rng = E.mulberry32((A.seed | 0) * 977 + run * 31 + 7);
  const pool = SHOTS.slice();
  // Fisher-Yates, so every run gets a genuine permutation rather than a rotation
  for (let i = pool.length - 1; i > 0; i--) {
    const j = Math.floor(rng() * (i + 1));
    const tmp = pool[i]; pool[i] = pool[j]; pool[j] = tmp;
  }
  // STUDIO is the slow orbit of the car itself. It is the shot that shows the upgrades, so
  // it is worth holding longer than a trackside cut, and never worth opening on.
  if (pool[0] === 'STUDIO') { pool[0] = pool[2]; pool[2] = 'STUDIO'; }
  return { shots: pool, hold: 3.4 + rng() * 2.2 };
}

track.enter = function () {
  const t = tierNow();
  const circ = SCR.world.makeCircuit(A.seed + st.i * 7, { tier: t });
...
track.update = function (dt) {
  ...
  track.camT += dt;
  const plan = track.plan || { shots: SHOTS, hold: 5 };
  // STUDIO orbits the stationary-looking car, so give it the extra beat it needs to get round
  const hold = plan.shots[track.cut] === 'STUDIO' ? plan.hold * 1.5 : plan.hold;
  if (track.camT > hold) {
    track.camT = 0;
...
// The circuit chip: which track this is, how long a lap is, and which camera is live.
// Without it a new layout every run reads as the same track filmed differently.
function paintCircuit() {
  if (!D() || !track.circ) return;
  const c = track.circ;
